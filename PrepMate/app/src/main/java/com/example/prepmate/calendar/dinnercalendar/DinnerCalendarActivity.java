...
public class DinnerCalendarActivity extends AppCompatActivity {
    ...
    private List<RecipeCalendar> dinnerRecipes; // List of snacks recipes
    private String selectedDate; // Holds the selected date
    private int userId; // Holds the user ID

    private static final String TAG = "DinnerCalendarActivity"; // Logging tag
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dinner_calendar);
        ...
        dinnerRecipes = new ArrayList<>(); // Initialize the recipe list
    ...
    private void loadDinnerRecipes() {
        ...
        if (cursor != null) {
                ...
                if (cursor.moveToFirst()) {
                    do {
                        ...
                        if (retrievedUserId == userId) { // Filter recipes by user ID
                            int dinnerId = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseHelper.COLUMN_DINNER_ID));
                            ...
                            dinnerRecipes.add(new RecipeCalendar(dinnerId, title, hours, minutes, "Dinner", retrievedUserId));
                        }
            ...
            Log.d(TAG, "No Dinner recipes found for user ID: " + userId);
        }

        // Set the adapter with the loaded recipes
        dinnerAdapter = new DinnerCalendarAdapter(dinnerRecipes, selectedDate, this, userId);
